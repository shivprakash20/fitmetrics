    ...
    <main className={styles.page}>
      <div className="container">
        <section className={styles.card}>
          <h1 className={styles.title}>{auth.verifyEmail.title}</h1>
          <p className={styles.subtitle}>{auth.verifyEmail.subtitle}</p>
          ...
          <form action={verifyEmailOtpAction} className={styles.form}>
            ...
            <div className={styles.row}>
              <label htmlFor="otp" className={styles.label}>{auth.verifyEmail.fields.otp.label}</label>
              <input id="otp" name="otp" inputMode="numeric" maxLength={6} className={styles.input} required />
            </div>
            <button type="submit" className={styles.submitBtn}>{auth.verifyEmail.submitButton}</button>
          </form>
